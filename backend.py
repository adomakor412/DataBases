...
def initiateTable():
    ...
    query2 = '''
        CREATE TABLE IF NOT EXISTS quantact (id INTEGER PRIMARY KEY AUTO_INCREMENT, first text, last text, number text, email text);
    '''
    cur.execute(query1)
    cur.execute(query2)
    conn.commit()
...
def delete(variable_id):
    conn = mysql.connector.connect(database="mydb", user ="root", password = "password", host="localhost")
    cur = conn.cursor()
    query = "DELETE FROM quantact WHERE id = "+str(variable_id)+";"
    cur.execute(query)
    #rows = cur.fetchall()
    conn.close()
...
def update(variable_id,first,last,number, email):
    conn = mysql.connector.connect(database="mydb", user ="root", password = "password", host="localhost")
    cur = conn.cursor()
    query = "UPDATE quantact SET first='"+first+"',last='"+last+"', number='"+number+"', email='"+email+"' where id = "+str(variable_id)+";"
    cur.execute(query)
    #rows = cur.fetchone()
    conn.close()
    #return search(first,last,number, email)
